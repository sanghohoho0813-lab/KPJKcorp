...
**도입 전후 비교 (Baseline)** — 도입 전 값은 시스템 안에 존재할 수 없으므로 대표가 6개 항목을 직접 입력한다(`settings.baseline`).
화면에는 항상 왼쪽 = **대표 입력값**, 오른쪽 = **시스템 실측값**으로 명시해 둘을 섞지 않는다. 표본 수를 함께 표시하고, 차이를 성과로 단정하지 않는다.

**왜 이 기록이 필요한가 (WHY_EVIDENCE)** — 자금조달·투자 심사 관점의 설명문을 `src/lib/evidence.ts` 한 곳에 두고 코치 카드·코치 화면·리포트가 같은 문장을 쓴다.
표현 원칙: "모든 기업이 그렇다" 같은 출처 없는 단정은 쓰지 않는다. 심사자가 근거를 물었을 때 답할 수 없는 문장은 신뢰를 깎는다.
