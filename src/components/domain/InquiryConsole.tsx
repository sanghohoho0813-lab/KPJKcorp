...
export function InquiryConsole({ embedded }: { embedded?: boolean }) {
  const st = useStore();
  ...
  const filterBar = (
    <SegmentedControl size="sm" value={filter} onChange={(k) => { setFilter(k); if (isMobile) setSel(null); }} options={[{ key: "open", label: `미답변 ${st.inquiries.filter((i) => i.status === "open" && (!me || i.assigneeId === me)).length}` }, { key: "answered", label: "답변완료" }, { key: "all", label: "전체" }]} />
  );

  return (
    <div>
      {embedded
        ? <div className="mb-4 flex flex-wrap items-center justify-between gap-2"><p className="text-[0.85rem] text-ink-2">고객 Portal 문의가 자동으로 이 큐에 등록됩니다. 답변하면 즉시 고객 화면에 반영됩니다.</p>{filterBar}</div>
        : <PageHeader title="문의 · 커뮤니케이션" desc="고객 Portal에서 들어온 문의가 자동으로 Queue에 등록됩니다. 답변하면 고객 Portal에 즉시 반영됩니다." actions={filterBar} />}
